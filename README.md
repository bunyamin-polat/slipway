...
2. **Write the example app, in this repo.** `template/app/` — a FastAPI service with one streaming endpoint and a static page, forty lines, no AI. Containerise it with `Dockerfile.fullstack` and run it through `run_local.py`. Deliberately trivial: the payload must never be the interesting part, and the blueprint must be provable without any other repository existing.
